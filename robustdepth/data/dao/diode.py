...
from robustdepth.data.data_meta import TFDataAccessObject, TESTING_ONLY_STR
...
class DIODETFDataAccessObject(TFDataAccessObject):
    ...
    def get_test_dataset(self):
        ...
        file_name_masks = [s.replace(b'.png', b'_depth_mask.npy') for s in file_name_images]

        def resize_imgs(img):
            return tf.image.resize(img, self.target_shape)

        def read_and_resize_depth(file_path):
            return np.ndarray.astype(resize(np.squeeze(np.load(file_path)), self.target_shape[:2]), np.float32)
        ...
        masks_ds = tf.data.Dataset.from_tensor_slices(file_name_masks).map(lambda loc_x: tf.numpy_function(read_and_resize_depth, [loc_x], Tout=tf.float32),
                                                                           num_parallel_calls=tf.data.experimental.AUTOTUNE)

        def mask_depth(depth, mask):
            return tf.cast(mask * depth, tf.float32)

        gts_ds = tf.data.Dataset.zip((masks_ds, gts_ds)).map(mask_depth, num_parallel_calls=tf.data.experimental.AUTOTUNE)
